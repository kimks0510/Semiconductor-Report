...
Get-ChildItem -LiteralPath $outputRoot -Filter '*-briefing.md' -File | ForEach-Object {
    ...
    if ($_.Name -match '^(\d{4}-\d{2}-\d{2})-briefing\.md$') {
        $entries.Add([PSCustomObject]@{ id = $Matches[1]; file = $_.Name; type = 'daily'; label = $Matches[1]; date = $Matches[1] })
    }
}
...
if (Test-Path -LiteralPath $termsPath) {
    ...
    $entries.Add([PSCustomObject]@{ id = 'terms'; file = 'terms.md'; type = 'glossary'; label = 'GSM 누적 용어 사전'; date = (Get-Item $termsPath).LastWriteTime.ToString('yyyy-MM-dd') })
}
...
if (Test-Path -LiteralPath $competitorPath) {
    ...
    $entries.Add([PSCustomObject]@{ id = 'competitor-timeline'; file = 'competitor-timeline.md'; type = 'competitor-timeline'; label = '경쟁사 타임라인'; date = (Get-Item $competitorPath).LastWriteTime.ToString('yyyy-MM-dd') })
}
...
if (Test-Path -LiteralPath $pricePath) {
    ...
    $entries.Add([PSCustomObject]@{ id = 'price-indicators'; file = 'price-indicators.md'; type = 'price-indicators'; label = '가격.공급 지표'; date = (Get-Item $pricePath).LastWriteTime.ToString('yyyy-MM-dd') })
}
...
if (Test-Path -LiteralPath $weeklyRoot) {
    Get-ChildItem -LiteralPath $weeklyRoot -Filter '*-rollup.md' -File | ForEach-Object {
        ...
        if ($_.Name -match '^(\d{4}-\d{2}-\d{2})-rollup\.md$') {
            $entries.Add([PSCustomObject]@{ id = "weekly-$($Matches[1])"; file = $_.Name; type = 'weekly-rollup'; label = "주간 GSM 롤업 $($Matches[1])"; date = $Matches[1] })
        }
    }
    Get-ChildItem -LiteralPath $weeklyRoot -Filter '*-interview-qa.md' -File | ForEach-Object {
        ...
        if ($_.Name -match '^(\d{4}-\d{2}-\d{2})-interview-qa\.md$') {
            $entries.Add([PSCustomObject]@{ id = "interview-$($Matches[1])"; file = $_.Name; type = 'interview-qa'; label = "모의 면접 Q&A $($Matches[1])"; date = $Matches[1] })
        }
    }
}
